...
function showStep(n) {
  // PROFESOR: Usamos classList para alternar .v-hidden. 
  // Al quitar .v-hidden, el elemento vuelve a su 'display: flex' original del CSS.
  if (n === 1) {
    document.getElementById('step1').classList.remove('v-hidden');
    document.getElementById('step2').classList.add('v-hidden');
  } else {
    document.getElementById('step1').classList.add('v-hidden');
    document.getElementById('step2').classList.remove('v-hidden');
  }
...
function syncCreateFormRequired(role) {
  ...
  const stepText = document.getElementById('userStepText');
  if (stepText) {
    if (isClient) stepText.classList.remove('v-hidden');
    else stepText.classList.add('v-hidden');
  }
...
window.openEditUserModal = function(id) {
  ...
  // PROFESOR: Aplicamos el mismo principio de .v-hidden aquí
  toggleEl('eu_group_company', !isCommercial);
  toggleEl('eu_group_phone', isCommercial);
  toggleEl('eu_group_type', !isCommercial);
  toggleEl('eu_group_commercial', !isCommercial);

  editUserBackdrop.classList.add('open');
};

// Utilidad pequeña para limpiar el código (DRY)
function toggleEl(id, show) {
  const el = document.getElementById(id);
  if (!el) return;
  if (show) el.classList.remove('v-hidden');
  else el.classList.add('v-hidden');
}
